...
namespace tmdb_api.endpoint.Controllers
{
    ...
    public class MovieController : ControllerBase
    {
        ...
        public async Task<IActionResult> Search(string searchText, int userId = 1)
        {
            ...
            try
            {
                var existingSearch = await _searchCriteriaService.GetSearchCriteriaBySerachTextAsync(searchText);

                if (existingSearch != null && existingSearch.Count > 0)
                {
                    var existingSearchData = await _movieService.GetMovies(existingSearch[0].Id);
